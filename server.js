...
app.get('/team_stats', function (req, res) {
    var football_Game = 'select * from football_games;';
    var wining = 'select count(*) from football_games where home_score > visitor_score;';
    var failing = 'select count(*) from football_games where home_score < visitor_score;';
    db.task('get-everything', task => {
        return task.batch([
            task.any(football_Game),
            task.any(wining),
            task.any(failing)
        ]);
    })
    .then(data => {
        res.render('pages/team_stats' ,{
            my_title: "Team Status",
            games: data[0],
            total_wining: data[1][0].count,
            total_failing: data[2][0].count
        })
    })
